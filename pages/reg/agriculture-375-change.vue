...
div
  ...
  lah-transition
    b-table#a375-table.text-center(
      v-if="committed",
      ref="table",
      caption-top,
      selectable,
...
export default {
  ...
  async fetch () {
    if (this.isBusy) {
      ...
      if (this.daysPeriod > this.warnDays) {
        const ans = await this.confirm(
          `搜尋區間大於${this.warnDays}天(過大區間，可能造成讀取時間過長而失敗)，請確認要執行？`
        )
        if (!ans) {
          return
        }
      } else if (
        this.$utils.empty(this.dateRange.begin) ||
        this.$utils.empty(this.dateRange.end)
      ) {
      ...
      this.getCache(this.cacheKey).then((json) => {
        ...
        if (this.forceReload !== true && json) {
          ...
          this.getCacheExpireRemainingTime(this.cacheKey).then((remaining) => {
            ...
            this.notify(
              `查詢成功，找到 ${this.rows.length} 筆375租約異動資料。`,
              { subtitle: `(快取) ${this.$utils.msToHuman(remaining)} 後更新` }
            )
          })
          this.committed = true
        } else {
  ...
  computed: {
    ...
    axiosType () {
      return this.qryType === 'land' ? '375_land_change' : '375_owner_change'
    },
    queryCount () {
  ...
  watch: {
    daysPeriod (val) {
      if (val < 1) {
        ...
        this.notify(`搜尋區間過大將造成伺服器回應緩慢(目前:${val}天)`, {
          title: '警告',
          type: 'warning',
          pos: 'tr'
        })
  ...
  methods: {
    ...
    landBuildNumber (item) {
      ...
      return this.$utils.empty(subNumber)
        ? mainNumber
