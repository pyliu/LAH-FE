<template lang="pug">
//- 外層維持 100vh 高度與 Flex 佈局
div.h-100.d-flex.flex-column.overflow-hidden
  //- 頂部 Header
  lah-header

  //- 內容區域佔滿剩餘空間，並在此層處理 Y 軸滾動 (overflow-auto)
  .flex-grow-1.position-relative.overflow-auto.scrollable-area.py-4
    lah-transition(appear, speed="fast")
      //- 移除強制的 h-100 與垂直置中，讓內容自然向下延伸
      b-container.main-container(fluid)

        //- 上方 Slogan 區域 (加上 flex-shrink-0 確保高度不足時圖片絕對不會被壓扁)
        .text-center.mb-4.anim-appear-1s.flex-shrink-0
          //- SVG Banner: 實驗室主題
          svg.mb-2.slogan-img(
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 400 140"
            preserveAspectRatio="xMidYMid meet"
            style="max-height: 22vh; width: 22.5vw; max-width: 600px; min-width: 250px;"
          )
            defs
        ...
        b-row.justify-content-center.px-3
          ...
          //- 10. 檔案應用預約申請 (權限控制)
          b-col(cols="12" sm="6" md="4" lg="3" xl="2" v-if="isAdm || isInf || authority.isAdmin").mb-4
          ...
          //- 11. 監控輪播 (權限控制)
          b-col(cols="12" sm="6" md="4" lg="3" xl="2" v-if="isInf || authority.isAdmin").mb-4
