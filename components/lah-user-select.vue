...
export default {
  ...
  computed: {
    filtered () {
      ...
      return Object.keys(this.siteUserNames).filter((id) => {
        return (id.match(idRE) && !this.siteUserNames[id].match(/^(登記|地價|測量|資訊|主任|秘書|第|服|會|地|TEST|HH|建)/gi)) ||
               (this.siteUserNames[id] && this.siteUserNames[id].match(nameRE))
      })
    }
  },
  watch: {
